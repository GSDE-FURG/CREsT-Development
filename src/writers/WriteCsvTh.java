...
import LogicSimulator.SignalExtendedMultipleFaults;
...
public class WriteCsvTh {
    ...
    public void writeCSVCompleteThMultipleFault(String filename, String circuitName, ArrayList <Signal> inputSignals,  ArrayList <Signal> outputSignals,  ArrayList <Signal> all_signals, String Head, ArrayList <ThreadFaultInjection> thread_list_test, String propagated_faults) throws IOException {
                ...
                for (int x=0, y=0; x< input_i.size() && y< item.size(); x++ , y++){
                    ...
                    if((x < input_i.size()) && (y < item.size())){
                        String s = input_i.get(x);
                           ...
                           // System.out.println("T: " + i + " fault: " + item.get(y).getFaultSignal() + " - v: "+ s + " faultfree: " + faultFree + " fault: " + fault + " - " + T );
                            for (int lines = 0; lines < item.size(); lines++) {
                                ArrayList<Signal> faultSignals_temp = item.get(y).getfaultSignalsList();
                                String faultSignals = "";
                                String faultSignalsBitFlips = "";
                                for (int mmm = 0; mmm < faultSignals_temp.size(); mmm++) {
                                    
                                    SignalExtendedMultipleFaults sigTemp = new SignalExtendedMultipleFaults(faultSignals_temp.get(mmm));
                                    
                                    faultSignals = faultSignals + "," + faultSignals_temp.get(mmm).getId();
                                    
                                    faultSignalsBitFlips = faultSignalsBitFlips + "," + sigTemp.getBitFlipExtended();
