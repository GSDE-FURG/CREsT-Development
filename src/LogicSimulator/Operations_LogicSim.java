...
class main_SPRMP_Exec {
    ...
    public void callMethodsAnalisys(Vector circuitPath, String library, String Reliability) throws Exception{
         ...
         Operations_LogicSim spr_mp_analysis = new Operations_LogicSim(Reliability, 0 , circuitPath.get(0).toString(), library);
         spr_mp_analysis.Simulator(library, Reliability);
                        
        //spr_mp_analysis.SPR_MP_FANOUTS(library, circuitPath.get(j).toString());
        //spr_mp_analysis.getInputFanouts(library, circuitPath.get(0).toString());
                ...
                System.out.println("======== Circuit : " + circuitPath.get(j));
                 for (int i = 0; i < TimeoutList.size(); i++) {
                    ...
                    System.out.println(i+1 + "  -  Timeout : " + timeout + "(s)");
                        Operations_LogicSim spr_mp_analysis = new Operations_LogicSim(Reliability, timeout , circuitPath.get(j).toString(), library);
                        //spr_mp_analysis.SPR_MP_FANOUTS(library, circuitPath.get(j).toString());
...
public class Operations_LogicSim {
    ...
    private long spr_timeConsumption;
     ...
     public Operations_LogicSim(String reliabilty,long Timeout, String circuitFilePath, String genlib) {
     ...
     public void SPR_MP_FANOUTS(String library, String CircuitFile)throws Exception {
        ...
        /*SPR Call*/
        Operations_LogicSim spr = new Operations_LogicSim(this.reliability, this.timeout,  CircuitFile, library);
     ...
     public void Simulator (String library, String CircuitFile) throws IOException, Exception{
        ...
        long loadTimeStart = System.nanoTime();//System.currentTimeMillis();
                ...
                ArrayList<InterLevel> level = this.levelCircuit.getInterLevels();
                
                System.out.println("T: "+level);
                
                System.out.println("- Printing Inter Logic Levels");
                
                for (int i = 0; i < level.size(); i++) {
                        InterLevel teste =  level.get(i);
                        
              
                        System.out.println("    -" + teste.getLevel() + " - " + teste.getInSingnals());
                }
                
                 ArrayList<GateLevel> gatesLevel = this.levelCircuit.getGateLevels();
                 
                  for (int i = 0; i < gatesLevel.size(); i++) {
                        GateLevel teste =  gatesLevel.get(i);
                        
                        System.out.println(" - " + teste.getGates() + " - " + teste.getIn());
                }
     }
}
