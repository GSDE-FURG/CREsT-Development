 ...
 * @author matheus
 */
public class SerialPTMOpsFloat {
    
    public static float getGateLevelMatrixIndex(ProbGateLevel pGateLevel, long x, long y, float prob) {
        ...
        if(xIndex > totalX || yIndex > totalY) {
            ...
            for (int i = 0; i < gates.size(); i++) {
                ...
                value = value * getPTMGateIndex(gates.get(i), xGateIndex, yGateIndex, prob);
    ...
    public static int getInterconnectionMatrixIndex(ProbInterLevel pInterLevel, long x, long y) {
        ...
        int inSignals = pInterLevel.getInSignals().size();                                
        int outSignals = pInterLevel.getOutSignals().size();
        ...
        for (int j = 0; j < inSignals; j++) {                
            dict.put(pInterLevel.getInSignals().get(j).getId(), Character.toString(binaryIn.charAt(j)));                
        }
        
        for (int j = 0; j < outSignals; j++) {
                aux = aux + dict.get(pInterLevel.getOutSignals().get(j).getId());
        }
    ...
    public static float getMultiplicationMatrixIndex(Object levelA, Object levelB, long x, long y, float prob) {
        
        if (levelA instanceof ProbInterLevel) {
            return getMultiplicationMatrixIndex((ProbInterLevel)levelA, (ProbGateLevel)levelB, x, y, prob);
        } else {
            if (levelB instanceof ProbInterLevel) {
                return getMultiplicationMatrixIndex((ProbGateLevel)levelA, (ProbInterLevel)levelB, x, y, prob);
            } else {
                return getMultiplicationMatrixIndex((ProbGateLevel)levelA, (ProbGateLevel)levelB, x, y, prob);
            }
        }
    }
    
    public static float getMultiplicationMatrixIndex(ProbInterLevel levelA,  ProbGateLevel levelB, long x, long y, float prob) {
        ...
        if(levelA.getOutSignals().size() != levelB.getIn()) {
            ...
            while(bigCounter < limit) {
                ...
                if(getInterconnectionMatrixIndex(levelA, x, bigCounter) == 1) {
                    value = value + getGateLevelMatrixIndex(levelB, bigCounter, y, prob);
                    break;
                }
    ...
    public static float getMultiplicationMatrixIndex(ProbGateLevel levelA,  ProbInterLevel levelB, long x, long y, float prob) {
        ...
        if(levelA.getOut() != levelB.getInSignals().size()) {
            ...
            while(bigCounter < limit) {
                ...
                if(getInterconnectionMatrixIndex(levelB, bigCounter, y) == 1) {
                    value = value + getGateLevelMatrixIndex(levelA, x, bigCounter, prob);
                }
                bigCounter += 1;
            }
        }                
    ...
    public static float getMultiplicationMatrixIndex(ProbGateLevel levelA, ProbGateLevel levelB, long x, long y, float prob) {
        ...
        if(levelA.getOut() != levelB.getIn()) {
            ...
            while(bigCounter < limit) {
                ...
                aux = getGateLevelMatrixIndex(levelA, x, bigCounter, prob);
                aux = aux * getGateLevelMatrixIndex(levelB, bigCounter, y, prob);
    ...
    public static float getMultiplicationMatrixIndex(ArrayList<Object> levels, long x, long y, float prob) {
        
        if(levels.size() == 1) {
            return getGateLevelMatrixIndex((ProbGateLevel)levels.get(0), x, y, prob);
            ...
            while(bigCounter < limit) {
                ...
                if (lastLevel instanceof ProbInterLevel) {
                    if(getInterconnectionMatrixIndex((ProbInterLevel)lastLevel, bigCounter, y) == 1) {
                        aux = getMultiplicationMatrixIndex(auxList, x, bigCounter, prob);
                    }
                } else {
                    
                    aux = getMultiplicationMatrixIndex(auxList, x, bigCounter, prob);
                    aux = aux * getGateLevelMatrixIndex((ProbGateLevel)lastLevel, bigCounter, y, prob);
                }
    ...
    public static float getPTMGateIndex(Object gate, int x, int y, float prob) {
    ...
    public static float getPTMCircuitIndex(ProbCircuit pCircuit, long x, long y, float prob) {
    ...
    public static float getCircuitReliability(ProbCircuit pCircuit, int[] itm, float prob) {
        float result = 0f;
        
        for (int i = 1; i < itm.length; i++) {
            result = result +  getPTMCircuitIndex(pCircuit, i-1, itm[i], prob);
